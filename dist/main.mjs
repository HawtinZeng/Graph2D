...
class Polygon {
   ...
   */
  computeNormal(edgeIdx) {
    const currentPoint = this.vertices[edgeIdx];
    const nextPoint = this.vertices[(edgeIdx + 1) % 4];
    const l = new Line$1(currentPoint, nextPoint);
  ...
  cut(multiline) {
    ...
    intersections.int_points1_sorted = getSortedArray(
    ...
    for (let int_point1 of intersections.int_points1_sorted) {
      if (
        int_point1.edge_before &&
        int_point1.edge_after &&
    ...
    for (let i = 1; i < intersections.int_points1_sorted.length; i++) {
      ...
      if (
        int_point1_curr.edge_before &&
        int_point1_curr.edge_before.bv === INSIDE$2
      ) {
        ...
        newEdges = newEdges
          .reverse()
          .map((edge) => new Flatten.Edge(edge.shape.reverse()));
